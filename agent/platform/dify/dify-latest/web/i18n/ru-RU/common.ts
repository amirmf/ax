const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Учетная запись AXAgent',
    langGeniusAccountTip: 'Ваша учетная запись AXAgent и связанные с ней пользовательские данные.',
  ...
  members: {
    ...
    invitationSentTip: 'Приглашение отправлено, и они могут войти в AXAgent, чтобы получить доступ к данным вашей команды.',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'Хостинговая служба OpenAI, предоставляемая AXAgent, позволяет вам использовать такие модели, как GPT-3.5. Прежде чем ваша пробная квота будет исчерпана, вам необходимо настроить других поставщиков моделей.',
  ...
  apiBasedExtension: {
    title: 'API-расширения обеспечивают централизованное управление API, упрощая настройку для удобного использования в приложениях AXAgent.',
    link: 'Узнайте, как разработать собственное API-расширение.',
    linkUrl: 'https://docs.axagent.automatx.ir/features/extension/api_based_extension',
  ...
  about: {
    ...
    nowAvailable: 'AXAgent {{version}} теперь доступен.',
    latestAvailable: 'AXAgent {{version}} - последняя доступная версия.',
  },
  appMenus: {
    overview: 'Мониторинг',
